  ...
  <div id="content-wrapper" class="d-flex flex-column">
    <div id="content">
      ...
      <div class="container-fluid complains-request">
        <h4 class="section-title">Complains/Request</h4>
        ...
        <div class="table-container">
          <table class="custom-table">
            ...
            <tbody>
              <?php
              if (!empty($_SESSION['complaints'])) {
                  foreach (array_reverse($_SESSION['complaints'], true) as $id => $entry) {
                      echo "<tr>
                          <td>{$entry['type']}</td>
                          <td>{$entry['subject']}</td>
                          <td>{$entry['date']}</td>
                          <td class='status pending'>{$entry['status']}</td>
                          <td class='action'>
                  ...
                  echo "<tr><td colspan='5'>No complaints or requests submitted yet.</td></tr>";
              }
              ?>
            </tbody>
          </table>
