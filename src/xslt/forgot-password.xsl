...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="forgot-password"
    select="document('../data/public/forgot-password.xml')/forgot-password" />
  ...
  <xsl:template match="/">
    <html>
      <head>
        ...
        <!-- PAGE TITLE -->
        <title>Forgot Password / FHLC</title>
      ...
      <body>
        <div class="container d-flex align-items-center justify-content-center"
          ...
          <!-- Centered Form -->
          <div class="col-lg-6">
            <form class="form-signin" method="POST" action="forgot-password-handler.php"
              autocomplete="off">
              <div class="text-center mb-4">
                <img src="{ $forgot-password/page/logo }" alt="Logo" width="100" height="100" />
                <h1 class="h3 mb-3 font-weight-normal">
                  <xsl:value-of select="$forgot-password/page/title" />
                </h1>
                <p class="text-muted">
                  <xsl:value-of select="$forgot-password/page/description" />
                </p>
              </div>

              <xsl:if test="$success_message != ''">
                <div id="forgot-success" class="alert alert-success" role="alert">
                  <xsl:value-of select="$success_message" disable-output-escaping="yes" />
                </div>
              </xsl:if>

              <xsl:if test="$error_message != ''">
                <div id="forgot-error" class="alert alert-danger" role="alert">
              ...
              <button class="btn btn-lg btn-primary btn-block btn-success" type="submit"
                id="reset-btn">Send Reset Link</button>
              ...
              <p class="mt-2 mb-1 text-muted text-center">
                <xsl:copy-of select="$forgot-password/page/call-to-register/node()"
                  disable-output-escaping="yes" />
              </p>
              <p class="mt-1 mb-3 text-muted text-center">
                <xsl:copy-of select="$forgot-password/page/call-to-login/node()"
                  disable-output-escaping="yes" />
                </p>
            </form>
          </div>
        ...
        <script>
          ...
          if (inputEmail) inputEmail.addEventListener('input', hideSuccess);
          });
        </script>
      </body>
    </html>
